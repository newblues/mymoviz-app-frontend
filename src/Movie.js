...
export default class Movie extends Component {
  ...
  handleClick() {
    ...
    if (isLike) {
      ...
      fetch('https://mighty-lowlands-31611.herokuapp.com/mymovies', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: `title=${this.props.movieName}&overview=${
          this.props.movieDesc
        }&poster_path=${this.props.movieImg}&idMovieDB=${this.props.idMovie}`
      }).catch(error => {
        console.error(error);
      });
    } else {
      ...
      fetch(
        `https://mighty-lowlands-31611.herokuapp.com/mymovies/${
          this.props.movieId
        }`,
        {
          method: 'DELETE'
